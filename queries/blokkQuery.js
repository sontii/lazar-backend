...
exports.blokkRange = async (req, res) => {
	...
	try {
		...
		const query = `SELECT DATE_FORMAT(datum, "%Y.%m.%d") AS datum, egyseg, FORMAT(sum(bteny_ert), 0) AS fogyar, FORMAT(sum(nteny_ert) - sum(nyilv_ert), 0) AS arres, CONCAT(FORMAT((sum(nteny_ert) - 
